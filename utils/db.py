...
def save_schedule_config(
    ...
    lookback_days: int = 7,
    # Jurisdiction track
    jur_enabled: bool = False,
    jur_days: str = "0,1,2,3,4",
    jur_time: str = "08:00",
    jur_domain: str = "both",
    jur_lookback: int = 7,
    # Enforcement track
    enf_enabled: bool = False,
    enf_interval_hours: int = 6,
    enf_lookback: int = 2,
) -> Dict[str, Any]:
...
def _compute_next_jur_run(days_csv: str, time_str: str) -> Optional[datetime]:
    ...
    try:
        ...
        for delta in range(8):
            ...
            candidate = target_date.replace(hour=hh, minute=mm, second=0, microsecond=0)
...
def update_enf_last_run() -> None:
    ...
    with get_session() as session:
        ...
        if row and row.enf_enabled:
            ...
            row.enf_next_run = datetime.utcnow() + timedelta(
                hours=row.enf_interval_hours or 6
            )
            session.commit()
