            ...
            items.append(
                {
                    "title": t.strip(),
                    "link": l.strip(),
                    "description": _strip_html(d).strip(),
                    "date": p.strip(),
                }
            )
...
class FTCEnforcementSource:
    ...
    NAME = "ftc"
    RSS_URL = (
        "https://www.ftc.gov/feeds/press-release.xml"  # confirmed working Mar 2026
    )
    ...
    def _extract_respondent(title: str) -> str:
        m = re.search(
            r"(?:vs?\.?\s+|against\s+|in the matter of\s+)(.+?)(?:\s*,|\s*\(|$)",
            title,
            re.IGNORECASE,
        )
...
class SECEnforcementSource:
    ...
    def fetch(self, lookback_days: int = 90) -> List[Dict]:
        results = []
        cutoff = datetime.utcnow() - timedelta(days=lookback_days)
        start_dt = (datetime.utcnow() - timedelta(days=lookback_days)).strftime(
            "%Y-%m-%d"
        )
            ...
            try:
                ...
                for hit in hits[:15]:
                    ...
                    title = (
                        ...
                        or src.get("period_of_report")
                        or "SEC Enforcement Filing"
                    )
                    blob = f"{title} {term}"
                    if not _is_enforcement_relevant(blob):
                        continue
    ...
    def fetch(self, lookback_days: int = 90) -> List[Dict]:
        ...
        for feed_url in self.FEEDS:
            try:
                ...
                if (
                    not raw
                    or raw.strip().startswith("<!DOCTYPE")
                    or "<html" in raw[:200].lower()
                ):
                    ...
                    results.append(
                        {
                            ...
                            "jurisdiction": "Federal",
                            "respondent": FTCEnforcementSource._extract_respondent(
                                item["title"]
                            ),
                            "summary": item["description"][:500],
                            "related_regs": _find_related_regs(blob),
...
class EEOCEnforcementSource:
    ...
    JSON_API = (
        "https://www.eeoc.gov/newsroom/search?keys=artificial+intelligence&format=json"
    )
            ...
            if rss:
                for item in _parse_rss_feed(rss):
                    ...
                    results.append(
                        {
                            "id": _action_id(
                                "EEOC", item.get("link", "") or item["title"]
                            ),
                            "source": self.NAME,
        ...
        for term in ["artificial intelligence", "algorithm", "automated hiring"]:
            try:
                ...
                for item in items:
                    ...
                    path = (
                        (attrs.get("path") or {}).get("alias") or attrs.get("url") or ""
                    )
                    link = (
                        ("https://www.eeoc.gov" + path)
...
class CourtListenerSource:
    ...
    def _normalise(self, result: Dict) -> Dict:
        ...
        return {
            ...
            "jurisdiction": "Federal",
            "respondent": case_name.split(" v. ")[-1][:100]
            if " v. " in case_name
            else "",
            ...
            "raw_json": {
                k: result.get(k)
                for k in ("caseName", "court", "dateFiled", "docketNumber", "snippet")
            },
        }
    ...
    def fetch_all(self, lookback_days: int = 90) -> Dict[str, Any]:
        ...
        for source in self.sources:
            ...
            try:
                ...
                log.info(
                    "%s: %d actions (%d new)", src_name.upper(), len(actions), new_cnt
                )
            except Exception as e:
        ...
        log.info(
            "Enforcement fetch complete: %d new, %d updated",
            counts["new"],
            counts["updated"],
        )
        return counts
