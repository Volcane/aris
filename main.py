...
@cli.command()
@click.option(
    "--days", default=30, show_default=True, help="Look back N days for new documents"
)
@click.option(
    "--limit", default=50, show_default=True, help="Max documents to summarize per run"
)
@click.option(
    "--domain",
    default="both",
...
@cli.command()
@click.option(
    "--limit", default=50, show_default=True, help="Max documents to summarize"
)
@click.option(
    "--force",
    is_flag=True,
...
@click.option(
    "--jurisdiction",
    default=None,
    help="Filter by jurisdiction: 'Federal' or state code",
)
@click.option(
    "--urgency",
    default=None,
...
@click.option("--days", default=30, show_default=True)
@click.option(
    "--output", default=None, help="Output file path (default: auto-named in ./output/)"
)
def export(fmt, days, output):
    ...
    console.print(
        f"[bold green]✓[/bold green] Exported to [underline]{path}[/underline]"
    )
...
def status():
    ...
    console.print(
        f"[bold]Enabled International[/bold]: {', '.join(ENABLED_INTERNATIONAL) or 'None'}"
    )
...
def diff(doc_id_a, doc_id_b):
    ...
    console.print(
        f"[dim]Comparing:[/dim] [bold]{doc_id_a}[/bold] → [bold]{doc_id_b}[/bold]\n"
    )
    orch = Orchestrator()
    ...
    if not result:
        console.print(
            "[yellow]No substantive differences found, or one/both documents not in database.[/yellow]"
        )
        return
...
@click.option("--days", default=30, show_default=True)
@click.option(
    "--severity", default=None, type=click.Choice(["Low", "Medium", "High", "Critical"])
)
@click.option(
    "--type",
    "diff_type",
...
def changes(days, severity, diff_type, unreviewed):
    ...
    if unreviewed:
        ...
        console.print(
            f"[bold]Regulatory Changes[/bold] — last {days} days ({len(diffs)} found)\n"
        )
...
def link(base_id, addendum_id):
    ...
    if not result:
        console.print(
            "[yellow]Could not complete analysis. Check that both document IDs exist in the database.[/yellow]"
        )
        return
...
def history(doc_id):
    ...
    console.print(f"\n[bold]{doc['title']}[/bold]")
    console.print(
        f"[dim]{doc['jurisdiction']} | {doc['doc_type']} | {doc['status']}[/dim]"
    )
    ...
    if links:
        ...
        for lnk in links:
            ...
            other = (
                lnk["related_doc_id"]
                if lnk["base_doc_id"] == doc_id
                else lnk["base_doc_id"]
            )
...
def _print_diff_result(d: dict):
    ...
    if modified:
        ...
        for r in modified:
            ...
            direction = (
                f"  [{r.get('direction')}]"
                if isinstance(r, dict) and r.get("direction")
                else ""
            )
...
@click.option(
    "--days", default=365, show_default=True, help="How far back to look for documents"
)
@click.option(
    "--no-conflicts",
    is_flag=True,
...
def synthesis_topics():
    ...
    if not suggestions:
        console.print(
            "[dim]No multi-jurisdiction document clusters found yet. "
...
def syntheses(limit):
    ...
    if not rows:
        console.print(
            '[dim]No syntheses yet. Run: python main.py synthesise "your topic"[/dim]'
        )
        return
    ...
    for r in rows:
        ...
        cons = (
            f"  [dim]{r['conflict_count']} conflicts[/dim]"
    ...
    console.print(
        '[dim]Run: python main.py synthesise "topic" --refresh  to re-run[/dim]'
    )
...
def _print_synthesis_result(result: dict):
    ...
    console.print(
        f"[dim]ID: {result.get('id')} · {result.get('docs_used', 0)} docs · "
    ...
    if obligations:
        console.print(
            f"[bold]Cumulative Obligations[/bold] ({len(obligations)} across all jurisdictions)"
        )
    ...
    if conflicts:
        console.print(
            f"[bold red]Jurisdiction Conflicts[/bold red] ({len(conflicts)} detected)"
        )
        for c in sorted(
            conflicts,
            ...
            console.print(
                f"  [dim]{c.get('jurisdiction_a')} vs {c.get('jurisdiction_b')} · {c.get('type')}[/dim]"
            )
...
@cli.command()
@click.option("--interval", default=24, show_default=True, help="Hours between runs")
@click.option(
    "--days", default=7, show_default=True, help="Lookback window for each run"
)
